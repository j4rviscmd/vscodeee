...
/**
 * Tauri implementation of the working copy backup tracker.
 *
 * Ported from the upstream Electron `NativeWorkingCopyBackupTracker`
 * (`electron-browser/workingCopyBackupTracker.ts`). Shows a save
 * confirmation dialog when closing with dirty files, using the
 * two-phase close handshake from `TauriLifecycleService` (issue #35).
 *
 * Key differences from the browser version:
 * - Async `onFinalBeforeShutdown()` that can show dialogs and await user input
 * - Hot exit configuration support (ON_EXIT, ON_EXIT_AND_WINDOW_CLOSE)
 * - Auto-save integration before shutdown
 * - Progress dialogs for long-running backup/save operations
 *
 * Resolves issue #38.
 */
...
export class TauriWorkingCopyBackupTracker extends WorkingCopyBackupTracker implements IWorkbenchContribution {

  static readonly ID = 'workbench.contrib.tauriWorkingCopyBackupTracker';
  ...
  private async handleModifiedBeforeShutdown(modifiedWorkingCopies: readonly IWorkingCopy[], reason: ShutdownReason): Promise<boolean> {
    ...
    try {
      ...
      return this.showErrorDialog(localize('backupTrackerConfirmFailed', 'The following editors with unsaved changes could not be saved or reverted.'), remainingModifiedWorkingCopies, error as Error, reason);
    }
  }
  ...
  private async shouldBackupBeforeShutdown(reason: ShutdownReason, modifiedWorkingCopies: readonly IWorkingCopy[]): Promise<readonly IWorkingCopy[]> {
    ...
    switch (reason) {
      ...
      case ShutdownReason.CLOSE:
        ...
        // Use TauriNativeHostService.getWindowCount() via INativeHostService
        // to determine if this is the last window being closed.
  ...
  private async discardBackupsBeforeShutdown(arg1: IWorkingCopyIdentifier[] | { except: IWorkingCopyIdentifier[] }): Promise<void> {
    ...
    // is important to not lose backups that have not
    // been handled.
