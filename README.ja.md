> [!IMPORTANT]
> Windowsにてアプリを起動しても拡張機能ロードされない問題を確認し修正中です(2026-05-09)。<br>
> fix済み。v0.22.0以上のアプリをインストールしてください。

> ---old---
> v0.6.0以下を利用している場合は、自動アップデート機能が実装されていません。<br>
> 手動でv0.9.0以上のアプリをインストールしてください。
>
> v0.1.0〜v0.8.0を利用している場合は、プロトコルハンドラとファイルウォッチャの初期化がメインスレッドを占有するため、ワークスペースなしでアプリ起動した場合にビジー状態になりフリーズする問題が生じています。<br>
> 手動でv0.9.0以上のアプリをインストールしてください（[#384](https://github.com/j4rviscmd/vscodeee/pull/384)で修正）。
...
| GitHub Copilot Chat       | チャット機能が Bun ランタイムと完全に互換でないため、Chat UI（チャットビューとアクティビティバータブ）は意図的に非表示。チャット以外の Copilot 機能（インライン補完、NES、SCM コミットメッセージ生成、認証）は正常に動作します。 | 全プラットフォーム — 詳細は [#468](https://github.com/j4rviscmd/vscodeee/pull/468) を参照。                                                                                                                |
